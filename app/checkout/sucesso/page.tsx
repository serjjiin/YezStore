...
function SucessoContent() {
  ...
  const [status, setStatus] = useState<'verifying' | 'paid' | 'pending' | 'error'>(() => {
    if (!paymentId || !externalRef) return 'error'
    return 'verifying'
  })
    ...
    <div style={{ textAlign: 'center', padding: '60px 24px', maxWidth: 400, margin: '0 auto' }}>
      ...
      {status === 'pending' && (
        <>
          ...
          <div style={{ fontSize: 13, color: 'var(--yez-gray)', lineHeight: 1.7, marginBottom: 24 }}>
            ...
            você receberá um e-mail e iniciaremos a preparação do seu pedido.
          </div>
      ...
      {status === 'error' && (
        <>
          ...
          <div style={{ fontSize: 13, color: 'var(--yez-gray)', lineHeight: 1.7, marginBottom: 24 }}>
            Seu pedido foi registrado. Estamos verificando o pagamento —
